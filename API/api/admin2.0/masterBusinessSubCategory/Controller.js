...
exports.addBusinessSubCategory = (req,res,next)=>{
    processData();
    async function processData(){
    var allbusinessSubCategory = await fetchBusinessSubCategoryMaster();
    var businessSubCategory = allbusinessSubCategory.filter((data)=>{
        if ( data.businessCategoryId == req.body.dropdownID && data.businessSubCategory.trim().toLowerCase() == req.body.fieldValue.trim().toLowerCase()) {
            return data;
        }
        })
        if (businessSubCategory.length > 0) {
            res.status(200).json({ duplicated : true });
        }else{
            const businessSubCategoryMaster = new BusinessSubCategoryMaster({
                                _id                    : new mongoose.Types.ObjectId(),
                                businessCategoryId     : req.body.dropdownID,
...
exports.getAllBusinessSubCategoryMaster = (req, res, next)=>{
    BusinessSubCategoryMaster.aggregate([
    {
    $lookup:
        {
...
exports.getBusinessSubCategory = (req, res, next)=>{
    ...
    BusinessCategoryMaster.findOne({ _id: category_id })
        .exec()
        .then(categoryData=>{
            ...
            .then(data=>{
                ...
                res.status(200).json({subcategoryData:data, businessCategoryName:categoryData.businessCategory});
            })
            .catch(err =>{
