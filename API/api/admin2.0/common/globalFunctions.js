...
exports.sendNotification = async (inputObj) => {
	...
	// console.log("sendNotification inputObj 	=> ", inputObj);
	...
	if(inputObj.role === 'admin'){
		...
		if(inputObj.userDetails){
			userContacts = {
							...
							mobileNumber 	: inputObj.toMobileNumber ? inputObj.toMobileNumber : inputObj.userDetails.profile.isdCode + inputObj.userDetails.profile.mobile,
							user_name 		: inputObj.userDetails.profile.fullName 
													? inputObj.userDetails.profile.fullName 
													: inputObj.userDetails.profile.firstname +" "+inputObj.userDetails.profile.lastname ,
	...
	if (userContacts && userContacts.success) {
		...
		const inAppNotifTemplate = await this.getNotificationTemplate(eventName, role, variables, 'Notification')
		...
		const emailTemplate = await this.getNotificationTemplate(eventName, role,
			{ ...variables, user_name: userContacts.user_name },
			'Email');
		// console.log("sendNotification emailTemplate => ", emailTemplate);
// console.log("globalVariable", globalVariable.SES_EMAIL )
		...
		const mobileTemplate = await this.getNotificationTemplate(eventName, role, variables, 'SMS')
...
insertNotification = (inputObj)=>{
   ...
   return new Promise((resolve,reject)=>{
	   notifications.save()
			.then(data=>{
			   resolve({success:true,message:"Notification Details Added",ID:data._id});
			})
			.catch(err =>{
			   console.log(err);
...
exports.createVideoConferencingRoom = (roomName, scheduledTime, user_Id, consultant_id,duration) => {
...
exports.rescheduleVideoConference = (roomId, newTime) => {
	...
	const basicAuth = `Basic NjI0ODI3YmI3OTdhNGYzZjljNTIxZTczOjV5NWVheWV5WWVNYVJ5RHlEdXF1SmVCdXp1R3U0dVVlU2VyZQ==`
	return axios
		.patch(`https://api.enablex.io/video/v2/rooms/${roomId}`, {
...
function removeTags(str) {
	if ((str === null) || (str === ''))
		...
		str = str.toString();
	return str.replace(/(<([^>]+)>)/ig, '').replace(/\&nbsp;/g, '').replace(/\&#39;/g, "'");
}
